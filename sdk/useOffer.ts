...
const discountPercentage = (
  { listPrice = 0, price = 0 }: { listPrice?: number; price?: number },
) => {
...
export const useOffer = (aggregateOffer?: AggregateOffer) => {
  ...
  const priceIsPix = offer?.priceSpecification?.filter((item) =>
    item.name === "Pix"
  )?.[0]?.price;
  const salePrice = offer?.priceSpecification?.filter((item) =>
    item.priceType === "https://schema.org/SalePrice"
  )?.[0]?.price;

  const installmentsWithInterest = offer?.priceSpecification
    ?.filter((item) =>
      item.priceComponentType === "https://schema.org/Installment" &&
      item.description?.includes("com juros")
    )
    .reduce(
      (acc: UnitPriceSpecification | null, curr: UnitPriceSpecification) => {
        ...
        if (currBillingDuration > accBillingDuration) {
          return curr;
        }

        if (
          curr.billingDuration === acc.billingDuration &&
  ...
  return {
    ...
    salePrice,
    discount: discountPercentage({ listPrice: listPrice?.price, price }),
    priceIsPix: salePrice && priceIsPix && priceIsPix < salePrice
      ? true
      : false,
  };
};
