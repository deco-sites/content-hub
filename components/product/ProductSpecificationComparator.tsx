...
import type {
  ...
  ProductWithComparator
} from "site/types/Product.d.ts";

interface Props {
  productSpecsComparator?: ProductSpecsComparator[];
  specs: ProductWithComparator["additionalProperty"];
}
...
export default function ProductSpecificationComparator({
  ...
  const specsMap = new Map(
    specs
      .filter(spec => spec.propertyID === "Este Produto inclui")
  ...
  return (
    <div class="flex w-full mt-4">
      <div class="flex w-full flex-col">
        {productSpecsComparator.map(({ name, icon }, idx) => {
          const hasSpec = specsMap.has(name);
